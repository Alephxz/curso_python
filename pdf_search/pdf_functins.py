...
def main():
    """ Main function to orchestrate the PDF downloading process."""
    #get_pdfs()
    pdf_dictonary = get_pdfs()
    print(pdf_dictonary.keys())
    main_dictionaty = {}
    for key, pdf_doc in pdf_dictonary.items():
        content = pdf_doc.content
        chunks_lenght = 20
        chunks = [content[i:i+chunks_lenght] for i in range(0, len(content), chunks_lenght)]
        for chunk in chunks:
            if chunk not in main_dictionaty:
                main_dictionaty[chunk] = [key]
            else:
                main_dictionaty[chunk].append(key)
    print(main_dictionaty.keys())
